// probe_ls5.js — E2E: pick an existing session, send a real prompt, capture streaming responses
const http = require("http");

const PORT = 50634;
const CSRF = "33403c54-8ec7-4ae3-82b5-2ce290a13da2";
const CASCADE_ID = "0e6dbe7a-cb29-4efe-9631-2ed23d3f0d3f"; // "Greeting Via Terminal ConnectRPC"
const PROMPT = "Dis bonjour en un seul mot";

function request(port, method, payloadHex, timeoutMs, onChunk) {
...
// metadata: {1:"Antigravity",2:"2.5.0",3:api_key,7:"2.5.0",8:"x86_64",12:"antigravity.remote",10:session_id}
...
function buildSendMessage(cid, text) {
  ...
  const config = Buffer.from([0x0a, 0x04, 0x08, 0xf6, 0x01, 0x12, 0x02, 0x08, 0x03]); // planner{1:246, 2:{1:3}}
...
(async () => {
  console.log(`Sending prompt to session ${CASCADE_ID}...`);
  const frame = buildSendMessage(CASCADE_ID, PROMPT);
  let frameCount = 0, bytes = 0;
  const resp = await request(PORT, "SendUserCascadeMessage", frame.toString("hex"), 120000, (chunk) => {
    bytes += chunk.length;
  });
  const fr = frames(resp.buf);
  frameCount = fr.length;
  console.log(`-> HTTP ${resp.status}, ${resp.buf.length} bytes, ${frameCount} data frames${resp.err ? " err=" + resp.err : ""}`);
  fr.slice(0, 5).forEach((f, i) => {
    console.log(`  frame[${i}] len=${f.length} hex=${f.subarray(0, 80).toString("hex")}`);
  });
  // try to find user/assistant text in last frame
  const last = fr[fr.length - 1];
  if (last) {
    const top = fields(last);
    console.log("  last frame top fields:", top.map((f) => `#${f.fnum}:${f.wt}=${f.wt === 0 ? f.v : f.bytes.length}B`).join(" "));
    for (const f of top) {
      if (f.wt === 2 && f.bytes.length > 20 && f.bytes.length < 5000) {
        const s = f.bytes.toString("utf8");
        if (/[A-Za-zÀ-ÿ]{3,}/.test(s) && !s.includes("file:///")) {
          console.log(`  text candidate in #${f.fnum}: ${JSON.stringify(s.substring(0, 200))}`);
        }
      }
    }
  }
