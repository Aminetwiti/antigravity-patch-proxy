...
const proxyErrorHistory = [];
function pushProxyErrorHistory(p) {
...
function isNotifyEnabled() {
    try {
        ...
        const ui = cfg.ui;
        if (ui && typeof ui.notifyEnabled === 'boolean')
            return ui.notifyEnabled;
    }
    catch {
        // fall through
    }
...
function notifyProxyError(p) {
    ...
    if (p.traceId === lastNotifiedTraceId && Date.now() - lastNotifiedAt < NOTIFY_DEDUP_MS)
        return;
...
function createWindow() {
    mainWindow = new electron_1.BrowserWindow({
        ...
        minWidth: 960,
        minHeight: 640,
        icon: path_1.default.join(getAssetsPath(), 'icon.png'),
...
electron_1.ipcMain.handle('ag:providers:fetch-models', async (_evt, params) => {
    try {
...
electron_1.ipcMain.handle('ag:providers:test', async (_evt, params) => {
    try {
        ...
        const doRequest = (targetUrl, method, body) => {
            return new Promise((resolve, reject) => {
        ...
        if (params.id) {
            try {
                ...
                if (parsed.providers && Array.isArray(parsed.providers)) {
                    const idx = parsed.providers.findIndex((x) => x.id === params.id);
                    if (idx !== -1) {
...
electron_1.ipcMain.handle('ag:config:set-notify', async (_evt, enabled) => {
    try {
        const cfgPath = getConfigPath();
        let cfg = {};
        if (fs_1.default.existsSync(cfgPath)) {
...
// ─────────────────────────────────────────────────────────────────────────────
electron_1.ipcMain.on('proxy:error', (_evt, payload) => {
    if (!payload || !payload.title)
        return;
    // Severity: 4xx other than 429 is "warn"; 5xx/timeout/auth/quota are "err".
    const sev = payload.status && payload.status >= 500
